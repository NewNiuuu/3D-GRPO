...
# All rights reserved.
#
# SpatialLM with a Qwen3 (Qwen3ForCausalLM) language backbone.
# This mirrors spatiallm_qwen.py (Qwen2) almost line-for-line; the only
# differences are the Qwen3 base classes and a flash-attn auto-fallback for
# the Sonata point encoder so the model can run without flash-attn installed.
...
# Detect flash-attn once. The Sonata encoder's SerializedAttention can run a
# manual (non-flash) attention path when flash_attn is unavailable, which keeps
# the model runnable on machines without a flash-attn build.
try:
...
class SpatialLMQwen3Config(Qwen3Config):
    model_type = "spatiallm_qwen3"


class SpatialLMQwen3ForCausalLM(Qwen3ForCausalLM):
    config_class = SpatialLMQwen3Config
    ...
    def __init__(self, config):
        ...
        if self.point_backbone_type == PointBackboneType.SCENESCRIPT:
            ...
            # Use flash attention in the point encoder only when it is available.
            # `enc_enable_flash` in point_config can force-enable/disable it.
            enc_enable_flash = point_config.get("enc_enable_flash", _HAS_FLASH_ATTN)
...
AutoConfig.register("spatiallm_qwen3", SpatialLMQwen3Config)
AutoModelForCausalLM.register(SpatialLMQwen3Config, SpatialLMQwen3ForCausalLM)
